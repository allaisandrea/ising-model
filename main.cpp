...
bool ParseArgs(int argc, const char* argv[], Arguments *args) {
    ...
    description.add_options()
      ...
      ("shape", value<std::vector<size_t>>(&(args->shape))->required()->multitoken())
...
void AppendToProtobuf(const Observables& obs, 
                      double stamp, 
                      pb::Simulation* sim) {
...
bool AtomicallyAcquired(uint8_t* node) {
    return __sync_fetch_and_or(node, 128) & 128;
}


bool Visited(uint8_t node) {
    return node & 128;
}


void MarkVisited(uint8_t* node) {
    (*node) |= 128;
}


void ClearVisitedFlag(uint8_t* node) {
    (*node) &= (~128);
}


bool Parallel(uint8_t node1, uint8_t node2) {
    return ((node1 ^ node2) & 1) == 0;
}


void Flip(uint8_t* node) {
    (*node) ^= 1;
}


template<size_t nDim, typename Node, typename Generator>
void FlipCluster(double prob,
                 const Index<nDim>& i0,
    ...
    const typename Generator::result_type iProb = rng->min() + 
    ...
    while (!queue->empty()) {
        ...
        for (size_t d = 0; d < nDim; ++d) {
            ...
            for(size_t dir = 0; dir < 4; dir +=2) {
                ...
                const bool add = !Visited(*node1) && 
                                 Parallel(*node0, *node1) && 
                                 (*rng)() > iProb;
                if (add) {
                    queue->emplace(i);
                    MarkVisited(node1);
...
template<size_t nDim, typename Node>
void Measure(const Index<nDim>& shape, Node* nodes, Observables* obs) {
    obs->representativeState = nodes[0];
    
    obs->stateCount.clear();
    obs->stateCount.resize(2, 0);
...
bool Serialize(const pb::Simulation& simulationPb, 
    ...
    if (!simulationPb.SerializeToOstream(&file)) {
        std::cerr << "Unable to save simulation protobuf to \"" 
                  << fileName << "\"." << std::endl;
        return false;
    }
    return true;
...
void run(const Arguments& args) {
    ...
    std::vector<uint8_t> nodes(GetSize(shape), 0);
    Index<nDim> i0;
    std::queue<Index<nDim>> queue;
    ...
    while (std::chrono::steady_clock::now() - startTime < 
        ...
        while (clusterSize < args.measureEvery * nodes.size()) {
            ...
            FlipCluster(args.prob, i0, shape, nodes.data(), 
                        &clusterSize, &rng, &queue);
            ClearVisitedFlag(nodes.data(), nodes.data() + nodes.size());
        }
        Measure(shape, nodes.data(), &observables);
    
        double stamp = (
            std::chrono::high_resolution_clock::now() - hiResStartTime).count();
