...
bool operator==(const UdhFileGroup::Entry &lhs,
                const UdhFileGroup::Entry &rhs) {
    return lhs.file_name == rhs.file_name && lhs.read_every == rhs.read_every;
}

std::ostream &operator<<(std::ostream &strm, const UdhFileGroup::Entry &x) {
    strm << "(" << x.file_name << "," << x.read_every << ")";
    return strm;
}
...
struct ParamsStruct {
    ...
    uint32_t n_wolff;
    uint32_t n_metropolis;
    uint32_t measure_every;
    uint32_t seed;
    std::string id;
};

std::pair<std::string, UdhParameters>
UdhParametersFromStruct(const ParamsStruct &params_struct) {
    UdhParameters params;
    params.set_j(params_struct.J);
    params.set_mu(params_struct.mu);
    params.set_n_wolff(params_struct.n_wolff);
    params.set_n_metropolis(params_struct.n_metropolis);
    params.set_measure_every(params_struct.measure_every);
    params.set_seed(params_struct.seed);
    params.set_id(params_struct.id);
    for (uint32_t x : params_struct.shape) {
        params.mutable_shape()->Add(x);
    }
    return {params_struct.file_name, params};
}

void TestMakeUdhFileGroupEntries(
    const std::vector<ParamsStruct> &test_case,
    const std::vector<std::vector<UdhFileGroup::Entry>> &expected_result) {
    std::vector<std::pair<std::string, UdhParameters>> pairs;
    for (const auto &item : test_case) {
        pairs.emplace_back(UdhParametersFromStruct(item));
    }
    EXPECT_EQ(MakeUdhFileGroupEntries(pairs), expected_result);
}

TEST(UdhFileGroup, MakeUdhFileGroupEntries) {
    // clang-format off
    TestMakeUdhFileGroupEntries({}, {});

    TestMakeUdhFileGroupEntries(
        {{"file1", 1.0, 2.0, {2, 3, 4}, 1, 1, 16, 1, "id1"}},
        {{{"file1", 1}}});

    TestMakeUdhFileGroupEntries(
        {{"file1", 1.0, 2.0, {2, 3, 4}, 1, 1, 16, 1, "id1"},
         {"file2", 1.0, 2.0, {2, 3, 4}, 1, 1,  8, 2, "id2"}},
        {{{"file1", 1}, {"file2", 2}}});

    TestMakeUdhFileGroupEntries(
        {{"file1", 1.0, 2.0, {2, 3, 4}, 1, 1, 16, 1, "id1"},
         {"file2", 1.0, 2.0, {2, 3, 4}, 1, 1,  4, 2, "id2"}},
        {{{"file1", 1}, {"file2", 4}}});

    TestMakeUdhFileGroupEntries(
        {{"file1", 1.0, 2.0, {2, 3, 4}, 1, 1, 16, 1, "id1"},
         {"file2", 1.0, 2.0, {2, 3, 5}, 1, 1,  8, 2, "id2"}},
        {{{"file1", 1}}, 
         {{"file2", 1}}});

    TestMakeUdhFileGroupEntries(
        {{"file1", 1.0, 2.0, {2, 3, 4}, 1, 1, 16, 1, "id1"},
         {"file2", 1.0, 2.0, {2, 3, 4}, 1, 1, 16, 2, "id2"},
         {"file3", 1.0, 2.0, {2, 3, 4}, 1, 1,  8, 3, "id3"}},
        {{{"file1", 1}, {"file2", 1}, {"file3", 2}}});

    TestMakeUdhFileGroupEntries(
        {{"file1", 1.0, 2.0, {2, 3, 4}, 1, 1, 16, 1, "id1"},
         {"file2", 1.0, 2.0, {2, 3, 4}, 1, 1, 16, 2, "id2"},
         {"file3", 1.0, 2.0, {2, 3, 5}, 1, 1,  8, 3, "id3"}},
        {{{"file1", 1}, {"file2", 1}}, 
         {{"file3", 1}}});

    TestMakeUdhFileGroupEntries(
        {{"file1", 1.0, 1.0, {2, 3, 4}, 1, 1, 16, 1, "id1"},
         {"file2", 1.0, 2.0, {2, 3, 4}, 1, 1, 16, 2, "id2"},
         {"file3", 1.0, 2.0, {2, 3, 4}, 1, 1,  8, 3, "id3"}},
        {{{"file1", 1}},
         {{"file2", 1}, {"file3", 2}}});
    ...
    TestMakeUdhFileGroupEntries(
        {{"file1", 1.0, 1.0, {2, 3, 4}, 1, 1, 16, 1, "id1"},
         {"file2", 1.0, 2.0, {2, 3, 4}, 1, 1, 16, 2, "id2"},
         {"file3", 1.0, 3.0, {2, 3, 4}, 1, 1,  8, 3, "id3"}},
        {{{"file1", 1}},
         {{"file2", 1}}, 
         {{"file3", 1}}});
    // clang-format on
