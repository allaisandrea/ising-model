...
template <size_t nDim>
using UdhTransitionProbsArrayD = std::array<UdhTransitionProbsD, 4 * nDim + 1>;

template <size_t nDim>
UdhTransitionProbsArrayD<nDim> ComputeUdhTransitionProbs2(const double J,
                                                          const double mu) {
    std::array<UdhTransitionProbsD, 4 * nDim + 1> result;
    const int64_t max_n_val = 2 * nDim;
    for (int64_t n = -max_n_val; n <= max_n_val; ++n) {
        const double p_d = std::exp(-J * n - mu);
        const double p_h = 1.0;
        ...
        UdhTransitionProbsD &tp = result.at(max_n_val + n);
        tp.p_dh = s * p_h;
        tp.p_hd = s * p_d;
        tp.p_hu = s * p_u;
        tp.p_uh = s * p_h;
...
void UdhMetropolisMove(
    const UdhTransitionProbsArrayD<nDim> &transition_probs_array,
    ...
    // Effect transition
    const UdhTransitionProbsD &transition_probs = transition_probs_array.at(n);
    UdhSpin &s = lattice[i];
    const double random_real =
        std::uniform_real_distribution<double>(0, 1)(*rng);
    if (s == UdhSpinDown()) {
        if (random_real < transition_probs.p_dh) {
            ++s.value;
        }
    } else if (s == UdhSpinUp()) {
        if (random_real < transition_probs.p_uh) {
            --s.value;
        }
    } else if (s == UdhSpinHole()) {
        if (random_real < transition_probs.p_hd) {
            --s.value;
        } else if (random_real <
                   transition_probs.p_hd + transition_probs.p_hu) {
            ++s.value;
        }
    }
}
